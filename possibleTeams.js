...
const historyBackup = require("./data/dataID.json");
...
const teamSelection = async (possibleTeams, matchDetails, quest) => {
    ...
    // console.log('quest custom option set as JSON:', JSON.parse(process.env.QUEST_PRIORITY.toLowerCase()))

    var priorityToTheQuest = JSON.parse(process.env.QUEST_PRIORITY.toLowerCase());

    // if(priorityToTheQuest && possibleTeams.length > 25 && quest && quest.total) {
    if(priorityToTheQuest && quest) {
        ...
        // console.log('play for the quest  filteredTeams',filteredTeams)
    ...
    if(!quest || quest.splinter === "snipe" || (quest.total - quest.completed == 0)){
        possibleTeams= possibleTeams.filter(team=>team[10]==='Ranked')
   }
    const res = await mostWinningSummonerTankCombo(possibleTeams, matchDetails);
    // console.log('Dont play for the quest, and play this:', res + "res0" + res[0] + "res1"+ res[1])
    if (res[0] && res[1]) {
        return { summoner: res[0], cards: res[1] };
    }
